...
cv::Vec<unsigned char, 3> read_pixel(cv::Mat& image, int x, int y) {
...
void colorize_multi_thread(cv::Mat image) {
    ...
    auto red_thread = std::async(std::launch::async, change_rows, std::ref(image), 255, 0, 0, 0, rows/3);

    auto blue_thread = std::async(std::launch::async, change_rows, std::ref(image), 0, 255, 0, rows/3, 2*rows/3);

    auto green_thread = std::async(std::launch::async, change_rows, std::ref(image), 0, 0, 255, 2*rows/3, rows);
